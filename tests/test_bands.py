...
class TestFindBand:
    ...
    def test_900_smr_downlink(self):
        assert "downlink" in find_band(935_012_500).name
        assert "900 MHz" in find_band(935_012_500).name

    def test_900_smr_uplink(self):
        assert "uplink" in find_band(896_012_500).name
        assert "900 MHz" in find_band(896_012_500).name
...
class TestDefaultStepHz:
    ...
    def test_900_smr_default_is_12500(self):
        # SMR raster is 12.5 kHz, not the 6.25 used at 700 MHz
        assert default_step_hz(935_000_000, 940_000_000) == 12_500
        assert default_step_hz(896_000_000, 901_000_000) == 12_500
    ...
    def test_start_band_wins_when_straddling(self):
        # 800 (12.5) → 900 SMR uplink; start side wins
